...
window.loadStorefrontData = async () => {
  try {
    const [prodSnap, secSnap, offSnap] = await Promise.all([
      fdb().collection('products').where('is_visible','==',true).orderBy('display_order').get(),
      fdb().collection('sections').orderBy('display_order').get(),
      fdb().collection('offers').where('is_visible','==',true).get(),
    ]);
    if (!prodSnap.empty) {
      window.PRODUCTS = prodSnap.docs.map(d => ({ ...d.data(), id: d.id }));
    }
    if (!secSnap.empty) {
      window.SECTIONS = ['All', ...secSnap.docs.map(d => d.data().name)];
    }
    if (!offSnap.empty) {
      window.OFFERS = offSnap.docs.map(d => ({ ...d.data(), id: d.id }));
    }
  } catch(e) {
